...
from controller.reporter import Reporter
...
class Session(object):
    ...
    def setfluidlist(self):
        self._fluidlist = self.DM.Query("ReferenceFluid", cols=["id", "name"])

    def setpipelists(self):
        self._tubinglist = self.DM.Query("ReferencePipe", cols=[
                                         "id", "name"], wcol="quality", wval="tubing")
        self._stainlesslist = self.DM.Query("ReferencePipe", cols=[
                                            "id", "name"], wcol="quality", wval="stainless")
        self._carbonlist = self.DM.Query("ReferencePipe", cols=[
                                         "id", "name"], wcol="quality", wval="carbon")
    ...
    def fluid(self, value):
        ...
        answerlist = self.DM.Query("ReferenceFluid", wcol="id", wval=value)
        ans = answerlist[0]
    ...
    def _setPipeWidgetList(self):
        ...
        for item in self.pipelist:
            a = self.DM.Query("ReferencePipe", cols=[
                              "name", "quality", "dnom_in", "dact_in"], wcol="id", wval=item[0])
            ans = a[0]
    ...
    def calculate(self):
        ...
        self.shortanswer = [self.answers[ideal - 2], self.answers[
            ideal - 1], self.answers[ideal], self.answers[ideal + 1], self.answers[ideal + 2]]

    def getResults(self):
        resultslist = []
        for row in self.shortanswer:
            rowlist = [row["ideal"], row["dnom"], int(row["ReynoldsNumber"]),
                       row["Flowregion"], round(row["dP100"], 2), round(row["velocity"], 2)]
    ...
    def MakeReport(self):
        R = Reporter()
        R.addTitle()
        R.addInputs(self.fluid, self.pipe)
        R.addGrid(self.getHeaders(), self.getResults())
        R.make()
